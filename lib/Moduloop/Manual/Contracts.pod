...
An invariant is an assertion that is run after every method in the interface, that defines one or more conditions that must
be met after the method has been called.

Invariants are specified using the C<invariants> key of a class/interface definition. The corresponding value is a hash
of description => subroutine pairs.
