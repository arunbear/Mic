...
The example counter in the L<Moduloop/SYNOPSIS> always starts at zero. Let's upgrade it to start from a user supplied value, so we can use it this way
...
We'll rewrite the counter example above and also show the more manual way of using Moduloop
    ...
    sub new {
        ...
        my $util = Moduloop::utility_class($class);
